...
export default function HeadCourses(){
  ...
  const getCourseDetails = (course) => {
    ...
    // Use actual data from mock.js
    const evaluationCount = courseEvaluations.length
    const enrollmentCount = course.enrolledStudents || 30 // Use enrolled students from course data

    const sentimentBreakdown = {
      positive: courseEvaluations.filter(e => e.sentiment === 'positive').length,
      neutral: courseEvaluations.filter(e => e.sentiment === 'neutral').length,
      negative: courseEvaluations.filter(e => e.sentiment === 'negative').length
    }

    // Calculate actual average ratings from evaluations
    const criteriaRatings = {
      'Content Quality': courseEvaluations.length > 0 
        ? (courseEvaluations.reduce((sum, e) => sum + (e.ratings?.clarity || 0), 0) / courseEvaluations.length).toFixed(1)
        : 'N/A',
      'Delivery Method': courseEvaluations.length > 0 
        ? (courseEvaluations.reduce((sum, e) => sum + (e.ratings?.usefulness || 0), 0) / courseEvaluations.length).toFixed(1)
        : 'N/A',
      'Assessment Fairness': courseEvaluations.length > 0 
        ? (courseEvaluations.reduce((sum, e) => sum + (e.ratings?.engagement || 0), 0) / courseEvaluations.length).toFixed(1)
        : 'N/A',
      'Support Provided': courseEvaluations.length > 0 
        ? (courseEvaluations.reduce((sum, e) => sum + (e.ratings?.organization || 0), 0) / courseEvaluations.length).toFixed(1)
        : 'N/A'
    }
                    ...
                    <div className="bg-white p-6 rounded-lg border">
                      ...
                      <div className="text-center">
                        <div className="text-5xl font-bold text-blue-600 mb-2">
                          ...
                          <span className="text-2xl text-gray-400">/4.0</span>
                        </div>
                      </div>
                    </div>
