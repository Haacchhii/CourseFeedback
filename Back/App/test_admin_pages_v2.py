"""
Comprehensive Admin Page Testing Script v2
Tests all admin endpoints and functionality with proper transaction handling
"""
...
class AdminPageTester:
    def __init__(self):
        ...
        # Create direct connection with proper transaction handling
        self.engine = create_engine(settings.DATABASE_URL)
    
    def get_connection(self):
        """Get a fresh connection for each test group"""
    ...
    def log_result(self, page: str, test_name: str, status: str, details: str = ""):
        ...
        if status == "PASS":
            ...
            self.results["issues_found"].append({"page": page, "test": test_name, "details": details})
        else:
            self.results["summary"]["warnings"] += 1
        
        icon = "✅" if status == "PASS" else "❌" if status == "FAIL" else "⚠️"
        print(f"  {icon} [{status}] {test_name}: {details[:100]}...")
    
    def run_query(self, conn, query, params=None):
        """Run a query safely with rollback on error"""
        try:
            if params:
    ...
    def test_admin_dashboard(self):
        ...
        try:
            ...
            # Test 4: Recent Evaluations - Check correct column name
            try:
                # First check evaluations table structure
                columns = self.run_query(conn, """
                ...
                timestamp_col = None
                for col in ['submission_date', 'submitted_at', 'created_at']:
                    if col in column_names:
                        timestamp_col = col
                        break
                
                if timestamp_col:
                    result = self.run_query(conn, f"""
                        SELECT e.id, e.{timestamp_col}, c.subject_name
                        FROM evaluations e
                        JOIN class_sections cs ON e.class_section_id = cs.id
                        JOIN courses c ON cs.course_id = c.id
                        ORDER BY e.{timestamp_col} DESC NULLS LAST
                        LIMIT 10
                    """).fetchall()
                    
                    self.log_result(page, "Recent Evaluations Query", "PASS", 
                        f"Retrieved {len(result)} recent evaluations (using {timestamp_col} column)")
                else:
                    self.log_result(page, "Recent Evaluations Query", "WARN", 
                        f"No timestamp column found. Available: {column_names}")
            except Exception as e:
            ...
            try:
                ...
                if result:
                    self.log_result(page, "Active Period Check", "PASS", 
                        f"Active period: {result[1]} (ID: {result[0]}, Status: {result[2]})")
                else:
            ...
            # Test 6: Evaluations Table Structure Analysis
            try:
                columns = self.run_query(conn, """
                ...
                self.log_result(page, "Evaluations Table Structure", "PASS", 
                    f"Columns: {column_info}")
            except Exception as e:
                self.log_result(page, "Evaluations Table Structure", "FAIL", str(e))
    ...
    def test_user_management(self):
        ...
        try:
            ...
            try:
                active = self.run_query(conn, "SELECT COUNT(*) FROM users WHERE is_active = true").scalar()
                inactive = self.run_query(conn, "SELECT COUNT(*) FROM users WHERE is_active = false").scalar()
            ...
            try:
                ...
                if result > 0:
                    list_count = self.run_query(conn, "SELECT COUNT(*) FROM enrollment_list").scalar()
        ...
        try:
            ...
            try:
                ...
                self.log_result(page, "Period Evaluation Counts", "PASS", 
                    f"Periods with evaluations: {periods_with_evals[:5] if periods_with_evals else 'None'}")
            except Exception as e:
                self.log_result(page, "Period Evaluation Counts", "FAIL", str(e))
            
            # Test 3: Period Enrollments Table Check
            try:
                result = self.run_query(conn, """
                    SELECT COUNT(*) FROM information_schema.tables 
                ...
                if result > 0:
                    count = self.run_query(conn, "SELECT COUNT(*) FROM period_enrollments").scalar()
            ...
            try:
                ...
                if result > 0:
                    count = self.run_query(conn, "SELECT COUNT(*) FROM period_program_sections").scalar()
    ...
    def test_enrollment_list(self):
        ...
        try:
            ...
            try:
                ...
                if result > 0:
                    count = self.run_query(conn, "SELECT COUNT(*) FROM enrollment_list").scalar()
    ...
    def test_database_schema(self):
        ...
        try:
            ...
            required_tables = [
                ...
                'enrollments', 'evaluations', 'evaluation_periods', 'audit_logs',
                'program_sections', 'section_students'
            ]
            ...
            for table in required_tables:
                try:
                    ...
                    if result > 0:
                        count = self.run_query(conn, f"SELECT COUNT(*) FROM {table}").scalar()
                        self.log_result(page, f"Table: {table}", "PASS", 
                            f"Exists with {count} rows")
                    else:
                        self.log_result(page, f"Table: {table}", "FAIL", 
                            "Table does not exist")
                except Exception as e:
                    self.log_result(page, f"Table: {table}", "FAIL", str(e))
    ...
    def run_all_tests(self):
        ...
        print("\n" + "="*70)
        print("🔍 COMPREHENSIVE ADMIN PAGE TESTING v2")
        print("="*70)
        ...
        if self.results["issues_found"]:
            ...
            for issue in self.results["issues_found"]:
                print(f"\n📌 {issue['page']} - {issue['test']}")
                print(f"   {issue['details'][:200]}...")
