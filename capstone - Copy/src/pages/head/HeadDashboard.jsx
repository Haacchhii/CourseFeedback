...
import { getCurrentUser, filterEvaluationsByAccess, filterCoursesByAccess, isSecretary, isDepartmentHead } from '../../utils/roleUtils'
...
export default function HeadDashboard() {
  ...
  useEffect(() => {
    ...
    if (currentUser && isDepartmentHead(currentUser)) {
      fetchEvaluations()
    }
  }, [currentUser])
  ...
  // Create courses from evaluations
  const accessibleCourses = useMemo(() => {
    const courseMap = new Map()
    accessibleEvaluations.forEach(evaluation => {
  ...
  const filteredEvaluations = useMemo(() => {
    return accessibleEvaluations.filter(evaluation => 
      filteredCourses.some(course => course.code === evaluation.course_code)
    )
  }, [accessibleEvaluations, filteredCourses])
  
  const total = filteredEvaluations.length
  const avg = total > 0 ? (filteredEvaluations.reduce((s,e)=> s + (e.overall_rating || 3), 0)/total).toFixed(2) : '0.00'
  ...
  const yearLevelSentiment = useMemo(() => {
    ...
    filteredCourses.forEach(course => {
      ...
      const courseEvals = filteredEvaluations.filter(e => e.course_code === course.code)
