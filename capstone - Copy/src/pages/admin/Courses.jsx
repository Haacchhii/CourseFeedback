...
export default function Courses() {
  ...
  // Get accessible courses and evaluations from API data
  ...
  // Get unique programs and year levels for filters
  ...
  const availableYearLevels = useMemo(() => {
    const yearLevels = [...new Set(accessibleCourses.map(course => course.yearLevel))]
    return yearLevels.sort()
  }, [accessibleCourses])

  // Filter courses based on search, program, and year level
  const filteredCourses = useMemo(() => {
    return accessibleCourses.filter(course => {
      ...
      const matchesYearLevel = yearLevelFilter === 'all' || course.yearLevel.toString() === yearLevelFilter
      
      return matchesSearch && matchesProgram && matchesYearLevel
    })
  }, [accessibleCourses, searchTerm, programFilter, yearLevelFilter])
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div>
              ...
              {isDepartmentHead(currentUser) ? (
                <input 
                  ...
                  readOnly
                  className="px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600 cursor-not-allowed w-full"
                  title="Department heads can only view courses for their assigned programs"
                />
              ) : (
                <select
                  value={programFilter}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Year Level Filter</label>
              <select
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredCourses.map((course) => {
                  const courseEvaluations = accessibleEvaluations.filter(e => e.courseId === course.id);
                  const participationRate = course.enrolledStudents ? Math.round((courseEvaluations.length / course.enrolledStudents) * 100) : 0;
                    ...
                    <tr key={course.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {courseEvaluations.length}/{course.enrolledStudents || 0} students
                        </div>
                        <div className="flex items-center mt-1">
                          <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
                            <div 
                              className={`h-2 rounded-full transition-all duration-300 ${
                                ...
                                participationRate >= 60 ? 'bg-yellow-500' :
                                'bg-red-500'
                              }`}
                          ...
                          <span className={`text-xs font-medium ${
                            participationRate >= 80 ? 'text-green-600' :
                            participationRate >= 60 ? 'text-yellow-600' :
                            'text-red-600'
                          }`}>
                            {participationRate}%
                          </span>
